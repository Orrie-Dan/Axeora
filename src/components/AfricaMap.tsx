...
import { africaCountries, africaCountryNodes, africaMapLabels } from "../data";
...
const NODES = [
  ...
  { cx: 50.6, cy: 56.9, r: 0.75 },

  { cx: 50.6, cy: 63.9, r: 0.9 },

  { cx: 18.2, cy: 27.4, r: 0.75 },

  { cx: 66.4, cy: 54.8, r: 0.9 },
...
export default function AfricaMap({
  ...
  const land = hero
    ...
    ? { from: "#2d5542", mid: "#214536", to: "#18382c", stroke: "rgba(197, 160, 89, 0.72)", terr: "rgba(197, 160, 89, 0.32)" }

    : { from: "#16406E", mid: "#102F53", to: "#0A2340", stroke: "#4B92DA", terr: "#3D7FC4" };

  const glow = hero ? "#c5a059" : "#17B3AC";

  const packet = hero ? "#48de93" : "#8FE6FF";

  const hub = hero ? "#c5a059" : "#31BDF2";

  const hubLabel = hero ? "rgba(255,255,255,0.72)" : "#9FC7EE";

  const labelFill = hero ? "rgba(255,255,255,0.58)" : "rgba(159,199,238,0.75)";

  const labelActive = hero ? "#48de93" : "#8fe6ff";
  ...
  const tooltipPos = useMemo(() => {
    ...
    const label = africaMapLabels[activeCountry as keyof typeof africaMapLabels];
  ...
  return (
    ...
    <div
      ...
      className={`africa-map-wrap africa-map-wrap--${tone}${hasFocus ? " has-focus" : ""} ${className}`.trim()}
      ...
      >
        ...
        <defs>
          ...
          <linearGradient id={`af-${uid}`} x1="0" y1="0" x2=".6" y2="1">
            ...
            <stop offset="0%" stopColor={land.from} />

            <stop offset="55%" stopColor={land.mid} />

            <stop offset="100%" stopColor={land.to} />

          </linearGradient>

          <linearGradient id={`lk-${uid}`} x1="1" y1="0" x2="0" y2="1">

            <stop offset="0%" stopColor={hero ? "#c5a059" : "#31BDF2"} />

            <stop offset="55%" stopColor={hero ? "#48de93" : "#17B3AC"} />
          ...
          <radialGradient id={`gl-${uid}`} cx=".5" cy=".5" r=".5">
            ...
            <stop offset="0%" stopColor={glow} stopOpacity=".2" />
        ...
        <g className="africa-map__terr" stroke={land.terr} strokeWidth=".24" opacity=".75">
        ...
        <g className="africa-map__pkt" fill={packet}>
        ...
        <g fill={hub}>
          ...
          {africaCountries.map((country, ci) => {

            const indices = africaCountryNodes[country] ?? [];
            ...
            const label = africaMapLabels[country];

            const displayName = label?.short ?? country;

            const anchor = label?.anchor ?? "middle";

            const lx = label?.x ?? NODES[indices[0]].cx;

            const ly = label?.y ?? NODES[indices[0]].cy - 3.5;
            ...
            return (
              ...
              <g
                ...
                className={`africa-map__country${isActive ? " is-active" : ""}${isDimmed ? " is-dimmed" : ""}`}

                style={{ transitionDelay: `${1.5 + ci * 0.08}s` }}

                role={interactive ? "button" : undefined}

                tabIndex={interactive ? 0 : undefined}

                aria-label={interactive ? `${country} — hover for details` : undefined}

                onMouseEnter={interactive ? () => highlight(country) : undefined}

                onMouseLeave={interactive ? () => highlight(null) : undefined}

                onFocus={interactive ? () => highlight(country) : undefined}

                onBlur={interactive ? () => highlight(null) : undefined}

                onClick={interactive ? () => select(country) : undefined}
              ...
              >
                ...
                {indices.map((i) => {
                  ...
                  const dotActive = activeNodes.has(i);

                  return (

                    <g key={i} className="africa-map__node">

                      <circle className="africa-map__hit" cx={node.cx} cy={node.cy} r="5" fill="transparent" />

                      <circle

                        className="africa-map__dot"

                        cx={node.cx}

                        cy={node.cy}

                        r={dotActive || isActive ? node.r * 1.7 : node.r}
                ...
                <text
                  ...
                  fill={isActive ? labelActive : labelFill}
        ...
        <circle cx="73.3" cy="12.8" r="3.8" fill="none" stroke={hub} strokeWidth=".26" opacity=".5" />

        <circle cx="73.3" cy="12.8" r="2.1" fill="none" stroke={hub} strokeWidth=".42" />

        <circle cx="73.3" cy="12.8" r="1.05" fill={hub} />
